    ...
    var body: some View {
            ...
            VStack(alignment: .leading) {
                ...
                Group {
                    Text(article.title)
                        ...
                        .fontWeight(.black)
                        .lineLimit(3)
                    Text("By \(article.author)".uppercased()) .font(.subheadline) .foregroundColor(.secondary)
                }
                .padding(.bottom, 0)
                .padding(.horizontal)
        ...
        .alert(isPresented: $showAlert) {
            Alert(title: Text("Reminder"),
                  ...
                  primaryButton: .default(Text("Yes"), action: { presentationMode.wrappedValue.dismiss() }), secondaryButton: .cancel(Text("No")))
        }
